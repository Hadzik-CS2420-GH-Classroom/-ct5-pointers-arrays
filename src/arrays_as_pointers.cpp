...
void printArray(int* arr, int size) {
    std::cout << "Inside printArray (received as pointer):" << std::endl;
...
void arraysAsPointers() {
    std::cout << "\n=== Treating Arrays as Pointers ===" << std::endl;

    int grades[] = {95, 87, 72, 91, 88};
    int size = 5;

    // --- 1. Array decay: passing an array to a pointer parameter ---
    std::cout << "\n--- 1. Array Decay to Pointer ---" << std::endl;

    // Notice: 'grades' is an array, but printArray expects a pointer (int*)
    ...
    std::cout << "\n--- 2. Pointer Arithmetic on Array Name ---" << std::endl;

    std::cout << "grades itself points to address: " << grades << std::endl;
    ...
    std::cout << "\nKey difference: arrays can't be reassigned:" << std::endl;
    std::cout << "  int* ptr = grades;  // OK - pointer can point to array" << std::endl;
    std::cout << "  ptr = nullptr;      // OK - pointer can be reassigned" << std::endl;
    std::cout << "  // grades = ptr;    // ERROR - array name can't be reassigned!" << std::endl;
}
